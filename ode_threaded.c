  ...
  while(i*neurons_per_thread < network->size)
    {
      ...
      thread_params[i]->f = f;
      thread_params[i]->t = t;
      rc = pthread_create(&(threads[i]), NULL, ode_update_neurons_thread, (void*)thread_params[i]);
      if(rc)
	{
...
void * ode_update_neurons_thread(void * thread_params)
{
  ...
  long num = params->num;
  long id = params->thread_id;
  const double * y = params->y;
  double * f = params->f;
  double t = params->t;
  ...
  long i, j, limit = start + num;
  if(network->size < limit)
    limit = network->size;

  //printf("* THREAD %ld: doing %ld to %ld\n", id, start, limit);
...
int ode_run_threaded(struct network * network, double t, double t1, double step_size, double error)
{
  ...
  while (t < t1)
	...
	{
	  printf("GSL_SUCCESS was not returned!\n");
	  break;
	}
    }
